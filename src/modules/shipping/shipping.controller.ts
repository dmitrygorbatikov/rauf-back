...
export class ShippingController {
	...
	@Post()
	public async plannedProduct(
		@Headers() headers,
		@Body() body: PlannedProductBodyDto,
	) {
		try {
			const { goods, carId, driverId, storageFrom, storageTo } = body
			const manager = await this.managerService.checkManagerRole(
				headers.token,
			)
			if (!manager) {
...
<<<<<<< HEAD
			...
			if (goodsWeightSum > ProductEnum.maxWeight) {
				return {
					error: `Goods weight can't be more than ${ProductEnum.maxWeight}`,
				}
			}
			if (car.carryingCapacity < goodsWeightSum) {
				return {
					error: `Goods weight can't be more than car carrying capacity(${car.carryingCapacity})`,
				}
			}
