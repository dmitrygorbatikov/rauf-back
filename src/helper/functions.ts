...
export const getRandomCarIndicators = (): CarIndicatorsDto => {
	...
	return {
		engineHeating: Math.floor(
			Math.random() * (maxEngineHeating - minEngineHeating) +
				minEngineHeating,
		),
		inflationOfTires: Math.floor(
			Math.random() * (maxInflationOfTires - mixInflationOfTires) +
				mixInflationOfTires,
		),
		tightnessOfBolts: Math.floor(
			Math.random() * (maxTightnessOfBolts - mixTightnessOfBolts) +
				mixTightnessOfBolts,
		),
		measurementDate: getRegisterDate(),
	}
